...
exports.getAllCollaborators = void 0;
...
const getAllCollaborators = async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.db)('collaborators')
            .select('*')
            .paginate({
        ...
        const sectorIds = Array.from(new Set(collaborators.map((c) => c.sector_id).filter(Boolean)));
        const userIds = Array.from(new Set(collaborators.map((c) => c.user_id).filter(Boolean)));
        const sectors = sectorIds.length
            ? await (0, connection_1.db)('sectors').whereIn('id', sectorIds).select('*')
            : [];
        const users = userIds.length
            ? await (0, connection_1.db)('users').whereIn('id', userIds).select('*')
            : [];
