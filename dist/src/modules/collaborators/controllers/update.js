...
const updateCollaborator = async (req, res) => {
    try {
        ...
        const hasExistingCollaborator = await (0, connection_1.db)('collaborators')
        ...
        const validation = create_1.createCollaboratorSchema.safeParse(req.body);
        ...
        const existingCollaborator = await (0, connection_1.db)('collaborators')
            .where(function () {
            this.where('registration', data.registration).orWhere('email', data.email);
        })
        ...
        if (Boolean(data.sector_id)) {
            const sector = await (0, connection_1.db)('sectors').where({ id: data.sector_id }).first();
            if (!sector) {
                return res.status(400).json({
        ...
        if (Boolean(data.user_id)) {
            const user = await (0, connection_1.db)('users').where({ id: data.user_id }).first();
            if (!user) {
                return res.status(400).json({
        ...
        await (0, connection_1.db)('collaborators')
            .where({ id: collaboratorId })
            .update({
            name: data.name,
