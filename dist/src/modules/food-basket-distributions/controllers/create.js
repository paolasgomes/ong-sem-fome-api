...
const createDistributionSchema_1 = require("../schemas/createDistributionSchema");
/**
 * @swagger
...
const createDistribution = async (req, res) => {
    try {
        const validation = createDistributionSchema_1.createDistributionSchema.safeParse(req.body);
        ...
        const basket = await (0, connection_1.db)('food_baskets')
        ...
        if (data.family_id) {
            const family = await (0, connection_1.db)('families').where({ id: data.family_id }).first();
            if (!family)
                return res.status(404).json({ error: 'Família não encontrada' });
        }
        if (data.campaign_id) {
            const campaign = await (0, connection_1.db)('campaigns')
