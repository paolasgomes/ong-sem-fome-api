...
exports.getAllDistributions = void 0;
...
const getAllDistributions = async (req, res) => {
    try {
        ...
        const result = await (0, connection_1.db)('food_basket_distributions')
            .select('*')
            .paginate({
        ...
        const formattedData = result.data.map(async (distribution) => {
            let collaboratorData = await (0, connection_1.db)('collaborators')
                .where({ id: distribution.collaborator_id })
                .first();
            if (!collaboratorData) {
                collaboratorData = null;
            }
            let familyData = await (0, connection_1.db)('families')
            ...
            if (distribution.campaign_id) {
                campaignData = await (0, connection_1.db)('campaigns')
            ...
            let basketData = await (0, connection_1.db)('food_baskets')
                .where({ id: distribution.food_basket_id })
                .first();
            const itemsBasketData = await (0, connection_1.db)('food_baskets_items')
                .where({ food_basket_id: distribution.food_basket_id })
                .select('*');
            const formattedItemsBasketData = await Promise.all(itemsBasketData.map(async (item) => {
                const productData = await (0, connection_1.db)('products')
            ...
            const { campaign_id, food_basket_id, collaborator_id, family_id, ...data } = distribution;
