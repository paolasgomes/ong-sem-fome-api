...
const updateFamily = async (req, res) => {
    try {
        const { id } = req.params;
        const hasExistingFamily = await (0, connection_1.db)('families').where({ id }).first();
        if (!hasExistingFamily) {
            return res.status(404).json({ error: 'Família não encontrada' });
        }
        const validation = create_1.createFamilySchema.safeParse(req.body);
        ...
        const existingFamily = await (0, connection_1.db)('families')
            .where(function () {
            this.where('responsible_cpf', data.responsible_cpf).orWhere('email', data.email);
        })
            .andWhereNot('id', Number(id))
            .first();
        if (existingFamily) {
            return res.status(400).json({
        ...
        const updatedFamily = await (0, connection_1.db)('families').where({ id }).update({
