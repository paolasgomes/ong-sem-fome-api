...
const createDonor = async (req, res) => {
    try {
        const validation = create_1.createDonorSchema.safeParse(req.body);
        ...
        const { type, name, email, phone, cpf, cnpj, street_number, street_complement, street_neighborhood, city, state, zip_code, street_address, observation, is_active, } = validation.data;
        const existingDonor = await (0, connection_1.db)('donors')
            .where(function () {
            if (cpf)
                this.orWhere('cpf', cpf);
            if (email)
                this.orWhere('email', email);
            if (cnpj)
                this.orWhere('cnpj', cnpj);
        })
            .orWhere('email', email)
            .first();
        if (existingDonor) {
        ...
        const [id] = await (0, connection_1.db)('donors').insert({
            type,
            name,
            email,
            phone,
