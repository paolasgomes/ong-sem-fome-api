...
exports.getDonationsReport = void 0;
const connection_1 = require("@/database/connection");
const getDonationsReport = async (req, res) => {
    try {
        const { type, donor_id, campaign_id, collaborator_id, product_id, created_from, created_to, } = req.query;
        const baseQuery = (0, connection_1.db)('donations');
        ...
        if (typeof collaborator_id === 'string' &&
        ...
        const summaryRow = (await baseQuery
            .clone()
            .select([
            connection_1.db.raw('COUNT(*) as total'),
            connection_1.db.raw('SUM(amount) as total_amount'),
            connection_1.db.raw('SUM(quantity) as total_quantity'),
        ])
            .first()) ?? {
            total: 0,
            total_amount: 0,
        ...
        return res.json({
            filters: {
                ...
                campaign_id: typeof campaign_id === 'string' ? Number(campaign_id) : null,
                collaborator_id: typeof collaborator_id === 'string' ? Number(collaborator_id) : null,
