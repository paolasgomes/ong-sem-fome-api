...
exports.getByIdFoodBaskets = void 0;
 ...
 */
const getByIdFoodBaskets = async (req, res) => {
    try {
        const { id: basketId } = req.params;
        const basket = await (0, connection_1.db)('food_baskets').where({ id: basketId }).first();
        if (!basket) {
            return res.status(404).json({
                error: 'Cesta de alimentos não encontrada',
            });
        }
        const basketItems = await (0, connection_1.db)('food_baskets_items')
...
exports.getByIdFoodBaskets = getByIdFoodBaskets;
