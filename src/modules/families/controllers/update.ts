...
const updateFamily = async (req: Request, res: Response) => {
  try {
    ...
    const hasExistingFamily = await db('families').where({ id }).first();
    ...
    const existingFamily = await db('families')
      .where(function () {
        this.where('responsible_cpf', data.responsible_cpf).orWhere(
          'email',
          data.email,
        );
      })
    ...
    const updatedFamily = await db('families').where({ id }).update({
