...
const getAllFoodBaskets = async (req: Request, res: Response) => {
  try {
    const { page, limit } = req.query;

    const result = await db('food_baskets')
      .select('*')
      .paginate({
    ...
    const formattedData = result.data.map(async (basket) => {
      const items = await db('food_baskets_items')
