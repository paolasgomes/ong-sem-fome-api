 ...
 */
const getCollaboratorsReport = async (req: Request, res: Response) => {
  try {
    const {
      name,
      registration,
    ...
    const query = db('collaborators')
      .leftJoin('sectors', 'collaborators.sector_id', 'sectors.id')
      .select<
        CollaboratorRow[]
      >('collaborators.id', 'collaborators.name', 'collaborators.registration', 'collaborators.email', 'collaborators.phone', 'collaborators.admission_date', 'collaborators.dismissal_date', 'collaborators.is_volunteer', 'collaborators.sector_id', 'collaborators.user_id', 'collaborators.is_active', 'sectors.name as sector_name');
    ...
    for (const c of collaborators) {
      ...
      current.total += 1;
      if (c.is_active) current.active += 1;
      if (c.is_volunteer) current.volunteers += 1;
    ...
    return res.status(200).json({
      filters: {
        ...
        sector_id:
          typeof sector_id === 'string' && !isNaN(Number(sector_id))
            ? Number(sector_id)
            : null,
        is_active:
          typeof is_active === 'string'
            ...
            : null,
        is_volunteer:
          typeof is_volunteer === 'string'
            ? ['true', 'false'].includes(is_volunteer.toLowerCase())
              ? is_volunteer.toLowerCase() === 'true'
              : null
            : null,
        admission_from:
          typeof admission_from === 'string' ? admission_from : null,
