...
const getStockReport = async (req: Request, res: Response) => {
  try {
    ...
    const query = db('products')
      ...
      .select<
        ProductRow[]
      >('products.id', 'products.name', 'products.unit', 'products.in_stock', 'products.minimum_stock', 'products.is_active', 'products.category_id', 'categories.name as category_name');
    ...
    const productsBelowMinimum = products.filter((product) => {
      if (
        typeof product.in_stock === 'number' &&
    ...
    const formattedProducts = products.map((product) => {
      ...
      const belowMinimum =
        minimumStock !== null ? inStock < minimumStock : false;
